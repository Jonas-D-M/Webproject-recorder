...
exports.default = (async () => {
    const { startPMServer, startStaticPMServer, startServer, stopPMServer } = server_1.default;
    ...
    try {
        ...
        const projectDir = posix_1.default.relative(actionDir, dir);
        ...
        await exec("git config --global user.email 'your-username@users.noreply.github.com'");
        await exec('git add .');
        await exec("git commit -am 'Generated showcase video' || echo 'No changes to commit'");
        await exec('git push');
        process.exit(0);
    }
    catch (error) {
